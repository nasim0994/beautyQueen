...
const MobileCategoryItems = ({ category, setMobileCategoryToggle }) => {
  ...
  if (category?.dropdown) {
          ...
          >
            {category.dropdown.map((subCategory) => (
              <li
                ...
                className="hover:text-primary duration-300 hover:bg-gray-100 p-1 rounded"
              >
                <Link
    ...
    return (
      <li
        onClick={() => setMobileCategoryToggle(false)}
        className="p-1 hover:text-primary duration-300"
      >
...
export default MobileCategoryItems;
